...
#' @param rr.digits,adj.rr.digits integer indicating significant digits used for
#'   \eqn{R^2} and adjusted \eqn{R^2} labels. Defaults preserve legacy behavior.
#' @param aic.digits,bic.digits integer indicating significant digits used for AIC
#'   and BIC labels. Defaults preserve legacy behavior.
#' @param eq.digits integer indicating significant digits used for polynomial
#'   equation coefficients. Default preserves legacy behavior.
...
stat_regline_equation <- function(
  ...
  rr.digits = 2, adj.rr.digits = rr.digits,
  aic.digits = 2, bic.digits = aic.digits, eq.digits = 2,
  ...
  layer(
    ...
    params = list(
      ...
      rr.digits = rr.digits, adj.rr.digits = adj.rr.digits,
      aic.digits = aic.digits, bic.digits = bic.digits, eq.digits = eq.digits,
...
StatReglineEquation <- ggproto("StatReglineEquation", Stat,
  ...
  compute_group = function(data, scales, formula, label.x.npc, label.y.npc,
                           ...
                           rr.digits, adj.rr.digits, aic.digits, bic.digits, eq.digits) {
    force(data)
...
.stat_lm <- function(formula, data, output.type = "expression", decreasing = TRUE,
  ...
  adj.rr <- summary(res.lm)$adj.r.squared %>% signif(adj.rr.digits)
  AIC <- stats::AIC(res.lm) %>% signif(aic.digits)
  BIC <- stats::BIC(res.lm) %>% signif(bic.digits)

  # Build model equation
  eq.char <- as.character(signif(polynom::as.polynomial(coefs), eq.digits), decreasing = decreasing)
